        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '预约单号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.changdiOrderUuidNumber" placeholder="预约单号" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           ...
                           href="http://localhost:8080/tiyuguanguanli/upload/changdiOrderMuBan.xls"
                        >批量导入场地预约数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('changdiOrder','导入导出')"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="changdiOrderPaymentTypes"
                                      header-align="center"
                                      label="支付类型">
                        <template slot-scope="scope">
                            {{scope.row.changdiOrderPaymentValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('changdiOrder','订单') && scope.row.changdiOrderTypes == 1 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-sold-out" size="mini" @click="refund(scope.row.id)">退款</el-button>
                            <el-button v-if="isAuth('changdiOrder','订单') && scope.row.changdiOrderTypes == 1 && sessionTable=='users'" type="primary" icon="el-icon-reading" size="mini" @click="deliver(scope.row.id)">同意</el-button>


                            <!--<el-button v-if="isAuth('changdiOrder','订单') && scope.row.changdiOrderTypes == 3 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-reading" size="mini" @click="receiving(scope.row.id)">使用</el-button>-->
    ...
    //当前表下拉框搜索条件
            ...
            json_fields: {
                //级联表字段
                     ...
                     '场地照片': 'changdiPhoto',
                     '场地类型': 'changdiTypes',
                     '预约原价格/天': 'changdiOldMoney',
                     '现价/天': 'changdiNewMoney',
                     ...
                     '余额': 'newMoney',
                //本表字段
                     '预约单号': "changdiOrderUuidNumber",
                     '实付价格': "changdiOrderTruePrice",
                     '订单类型': "changdiOrderTypes",
                     '支付类型': "changdiOrderPaymentTypes",
                     '预约日期': "yuyueTime",
                     '订单创建时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :"changdi_order",//当前表表名,
                        sumColum : 'changdi_order_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'changdi_order_name',//分组字符串字段
                        // types : 'changdi_order_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "changdi_order_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.changdiOrderUuidNumber!= '' && this.searchForm.changdiOrderUuidNumber!= undefined) {
                    params['changdiOrderUuidNumber'] = '%' + this.searchForm.changdiOrderUuidNumber + '%'
                }
			...
			deliver(id){
				this.$confirm(`确定要同意吗?`, "提示", {
				   ...
				   this.$http({
				        url: "changdiOrder/deliver?id="+id,
				        method: "post",
				    }).then(({ data }) => {
				        if (data && data.code === 0) {
				            this.$message({
			...
			receiving(id){
				this.$confirm(`确定要使用吗?`, "提示", {
				   ...
				   this.$http({
				        url: "changdiOrder/receiving?id="+id,
				        method: "post",
				    }).then(({ data }) => {
				        if (data && data.code === 0) {
				            this.$message({
			...
			commentback(id){
				...
				this.$confirm(`确定要评价吗?`, "提示", {
				   ...
				   this.$http({
				        url: "changdiOrder/commentback?id="+_this.commentbackId+"&commentbackText="+_this.commentbackContent,
                        method: "post",
                        }).then(({ data }) => {
				        if (data && data.code === 0) {
				            this.$message({
